...
def switchToX(x, parListDict):
    ''' 
    0 = in use, 1 not used
    
    parListDict is a list of parameter dictionaries from bw2, ideally retrieved from searchParameters function
    '''
    for i in range(len(parListDict)):
        parListDict[i]['amount']=1 if i != x else 0
...
## Biochar - inventory through a supply-chain
def how_much_biochar(activity=tuple(('use_2_green-roof', 'b33b9e50870c449ba0607b6f21a52898')), amount=1):
    ...
    References:
        Found in file ```multilca.py``` in AB package - explains how to get the technosphere flows
...
def how_much_of_x_in_y(x, y, amount=1):
    '''
    Calculates how much biochar (in kg) was used in a given activity (and it's upstream supply chain)
    Tto be used with lifecycle of final products or cradle-to-gate manufacturing of products
    - x : list of activities, as tuple, to search for
    - y : single activity investigated
    - amount: 1 (default)
    
    Returns: sum of biochar in kg, and biochar by types available in the database
    
    USE:    
        total, df = how_much_of_x_in_y()
        print("Total amount of product: % 5.5f kg " %(total) )
        df
    ...
    '''
    # calculate inventory
    lca = bw2.LCA({y:amount})
    lca.lci()
    # calculate technosphere flows
    technosphere_flows = np.multiply(lca.supply_array, lca.technosphere_matrix.diagonal())
    
    # x processes names and their ids in the matrices
    xactivities = {bw2.get_activity(a).key:bw2.get_activity(a)['name'] for a in x}
    
    xactivities_ids = {key:lca.activity_dict[key] for key in xactivities.keys() }
    
    # calculate how much there is
    _amounts = {key:{'name': xactivities[key],
                            'amount': technosphere_flows[xactivities_ids[key]]}
                       for key in xactivities.keys() }
    
    df = pd.DataFrame.from_dict(_amounts, orient='index')
    df.set_index('name', inplace=True, drop=True)
...
## Calculate multiple LCIA with different parameter values
mrg_el = [0,1,2]
mrg_heat = [0,1,2]
params={'mrg_el': mrg_el,
        'mrg_heat': mrg_heat}
fus = [{('pro_biochar', '6bedad3b18634235a2f69378c4981e91_copy5'): 510.0},
       {('pro_biochar', '6bedad3b18634235a2f69378c4981e91_copy10'): 250.0},
      ]
def calculateLCIA_withDifferentParams(fus, 
                                      method = tuple(('IPCC 2013', 'climate change', 'GWP 100a')),
                                      **params):    
    many_scores = {}
    for n,m in enumerate(itertools.product(fus, *params.values() )):
        print(n, m[0], m[1], m[2])
        # n = counter, m[0] = fu, m[j] = value param j, with j > 0
        param_names = list(params.keys())
        # update parameters
        for j in range(1, len(m)+1):
            wanted = []
            wanted = searchParameters( param_names[j-1] )
            wanted = switchToX(m[j], wanted)

            bw2.parameters.new_project_parameters(wanted, overwrite=True)

        # needed, for all relationships to be recalculated 
        bw2.parameters.recalculate() 
        ...
        lca.lcia()
        many_scores[n] = {}
        many_scores[n]['score'] = lca.score
        many_scores[n]['fu'] = m[0]
        for j in range(1, len(m)+1):
            many_scores[n][param_names[j-1]] = m[j]
